    ...
    <section id="experience" className="px-6 py-28">
      <div ref={ref} className="max-w-5xl mx-auto">
        ...
        >
          ...
          <div
            className="flex md:hidden gap-2 mb-8 overflow-x-auto pb-1"
          ...
          >
            {experience.map((exp, i) => (
              <button
                key={i}
                onClick={() => setActive(i)}
                className="shrink-0 px-4 py-2 rounded-full font-mono text-sm font-medium border transition-all"
                style={
                  active === i
                    ? { color: "#FB923C", borderColor: "#FB923C", background: "#FB923C12" }
                    : { color: "var(--fg-muted)", borderColor: "var(--border)", background: "transparent" }
                }
              >
                {exp.company}
              </button>
          ...
          {/* ── Desktop: vertical tabs + content side by side ── */}
          <div className="hidden md:flex gap-0 min-h-[340px]">
            {/* Left tabs */}
            <div
          ...
          <div className="md:hidden">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                transition={{ duration: 0.22, ease: "easeInOut" }}
              >
                <h3 className="text-xl font-semibold mb-1.5 leading-tight" style={{ color: "var(--fg)" }}>
                  {current.role}{" "}
                  <span className="text-[#FB923C]">@ {current.company}</span>
                </h3>
                <p className="font-mono text-xs uppercase tracking-widest mb-6" style={{ color: "var(--fg-muted)" }}>
                  {current.dates}
                </p>
                <ul className="space-y-4">
                  {current.bullets.map((bullet, i) => (
                    <li key={i} className="flex gap-3 text-base leading-relaxed" style={{ color: "var(--fg-2)" }}>
                      <span className="text-[#FB923C] mt-1 shrink-0 text-xs">▷</span>
                      {bullet}
                    </li>
                  ))}
